...
class Test(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.synth_foos = [synth.circle, synth.ellipse, synth.hyperbola, synth.parabola, synth.line]

        # positional arguments
        self.circle_args = {"radius": self.radius,
                            "center": self.center,
                            "n": self.n}
        self.ellipse_args = {"semi_x_axis": self.semi_x_axis,
                             "semi_y_axis": self.semi_y_axis,
                             "center": self.center,
                             "n": self.n}
        self.hyperbola_args = {"semi_x_axis": self.semi_x_axis,
                               "semi_y_axis": self.semi_y_axis,
                               "center": self.center,
                               "n": self.n}  # TODO questo è ovviamente tutto cannato, devo ancora implementarla.
        self.line_args = {"point_1": self.point_1,
                          "point_2": self.point_2,
                          "n": self.n}
        self.outliers_args = {"x_range": self.x_range,
                              "y_range": self.y_range,
                              "n": self.n}
        self.parabola_args = {"a": self.a,
                              "b": self.b,
                              "c": self.c,
                              "n": self.n}
        ...
        self.args_with_outliers_and_shuffle_true = {"outliers_perc": self.outliers_perc,
                                                    "shuffle": True}
    ...
    def _retrieve_positional_args(self, foo):
        ...
        elif foo.__name__ == 'hyperbola':
            return self.hyperbola_args
    ...
    def _output_shapes_are_correct(self, foo):
        ...
        def _are_shapes_pair_output_valid(foo, **kwargs):
            for homogeneous in [False, True]:
                ...
                if (transformation_points.shape[0] + outliers_points.shape[0] != numpoints
                        or transformation_points.shape[1] != n_coords
                        or outliers_points.shape[1] != n_coords):
                    return False
            return True

        single_output_scenarios = [self.args_with_outliers_and_shuffle_true, self.args_without_outliers]
        pair_output_scenarios = [self.args_with_outliers_and_shuffle_false]

        for scenario in single_output_scenarios:
            if not _is_shape_single_output_valid(foo, **self._retrieve_positional_args(foo), **scenario):
                print("Shape inconsistency in function {}, scenario: {}".format(foo.__name__, scenario))
                return False

        for scenario in pair_output_scenarios:
            if not _are_shapes_pair_output_valid(foo, **self._retrieve_positional_args(foo), **scenario):
                print("Shape inconsistency in function {}, scenario: {}".format(foo.__name__, scenario))
                return False
    ...
    ****************************************************************************************************************"""
    ...
    def test_outliers_behavior(self):
        ...
        xs = output[:, 0]
        ys = output[:, 1]
        self.assertTrue(all(xs >= self.x_range[0])
    ...
    def test_outliers_output_shape(self):
        ...
        for homogeneous in [False, True]:
            output = synth.outliers(x_range=self.x_range, y_range=self.y_range, n=self.n, homogeneous=homogeneous)
            n_coords = 3 if homogeneous else 2
    ...
    ****************************************************************************************************************"""
    ...
    def _all_points_are_in_circle(self, points):
        ...
        xs = points[:, 0]
        ys = points[:, 1]
        res = np.allclose((xs - self.center[0])**2 + (ys - self.center[1])**2, np.repeat(self.radius**2, xs.shape[0]))
        return res

    def test_circle_shuffle_true_and_outliers(self):
        """ assert that not all points are explained by the circle equation """
        points = synth.circle(**self.circle_args,
                              **self.args_with_outliers_and_shuffle_true)
        self.assertFalse(self._all_points_are_in_circle(points))

    def test_circle_shuffle_false(self):
    ...
    ****************************************************************************************************************"""
    ...
    def _all_points_are_in_ellipse(self, points):
        ...
        b = self.semi_y_axis
        x_0, y_0 = self.center
        res = np.allclose((xs - x_0)**2/a**2 + (ys - y_0)**2/b**2, np.ones(self.n))
        return res

    def test_ellipse_shuffle_true_and_outliers(self):
        """ assert that not all points are explained by the circle equation """
        points = synth.ellipse(**self.ellipse_args,
                               **self.args_with_outliers_and_shuffle_true)
        self.assertFalse(self._all_points_are_in_ellipse(points))

    def test_ellipse_shuffle_false(self):
        """ assert that all points are explained by the circle equation """
        points = synth.ellipse(**self.ellipse_args,
                               shuffle=False)
    ...
    ****************************************************************************************************************"""
    ...
    def _all_points_are_in_hyperbola(self, points):
    ...
    def test_hyperbola_shuffle_true_and_outliers(self):
        """ assert that not all points are explained by the circle equation """
        points = synth.hyperbola(**self.hyperbola_args,
                                 **self.args_with_outliers_and_shuffle_true)
        self.assertFalse(self._all_points_are_in_hyperbola(points))

    def test_hyperbola_shuffle_false(self):
        """ assert that all points are explained by the circle equation """
        points = synth.hyperbola(**self.hyperbola_args,
                                 shuffle=False)
        self.assertTrue(self._all_points_are_in_hyperbola(points))
    ...
    ****************************************************************************************************************"""
    ...
    def _all_points_are_in_line(self, points):
        ...
        m = (y1 - y2) / (x1 - x2)
        q = y1 - m * x1
        res = np.allclose(m * xs + np.repeat(q, self.n), ys)
        return res

    def test_line_shuffle_true_and_outliers(self):
        """ assert that not all points are explained by the circle equation """
        points = synth.line(**self.line_args,
                            **self.args_with_outliers_and_shuffle_true)
        self.assertFalse(self._all_points_are_in_line(points))

    def test_line_shuffle_false(self):
        """ assert that all points are explained by the circle equation """
        points = synth.line(**self.line_args,
                            shuffle=False)
    ...
    ****************************************************************************************************************"""
    ...
    def _all_points_are_in_parabola(self, points):
        ...
        xs = points[:, 0]
        ys = points[:, 1]
        res = np.allclose(self.a * xs**2 + self.b * xs + np.repeat(self.c, self.n), ys)
        return res

    def test_parabola_shuffle_true_and_outliers(self):
        """ assert that not all points are explained by the circle equation """
        points = synth.parabola(**self.parabola_args,
                                **self.args_with_outliers_and_shuffle_true)
        self.assertFalse(self._all_points_are_in_parabola(points))

    def test_parabola_shuffle_false(self):
        """ assert that all points are explained by the circle equation """
        points = synth.parabola(**self.parabola_args,
                                shuffle=False)
